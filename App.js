...
import DistanceWidget from './widgets/NotePad'; // Add this line
function App() {
  const [widgets, setWidgets] = useState([
    ...
    { id: new Date().getTime() + 3, component: <WeatherWidget />, area: 'right-widget', name: "Weather" },
    { id: new Date().getTime() + 4, component: <DistanceWidget />, area: 'left-widget', name: "Distance" }, // Add this line
  ]);
  ...
  const removeWidget = (index) => {
    try {
      setWidgets(widgets.filter(widget => widget.id !== index && widget.name !== "Weather"));
    } catch (error) {
      console.log(error);
    }
  };
